...
{
	//Setup digital pins
	DDRD |= (1<<DDD2); //set up PD2 to D/C pin
	LCD_DDR |= (1<<LCD_RST)|(1<<LCD_TFT_CS)|(1<<LCD_MOSI)|(1<<LCD_SCK);	//Set up output pins
...
void sendCommands (const uint8_t *cmds, uint8_t length)
{
	...
	{
		//clear(PORTD, LCD_DC);	//D/C pulled low for command
		PORTD &= ~(1<<PORTD2); //clear D/C to send command
		
		SPI_ControllerTx_stream(*cmds++);
		
		numData = *cmds++;	// # of data bytes to send

		//set(PORTD, LCD_DC);	//D/C set high for data
		PORTD |= (1<<PORTD2); //set D/C to sednd data
